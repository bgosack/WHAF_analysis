...
# outputs
outWS = r'D:\Users\begosack\Documents\WHAF\Index_Development\Connectivity\Aquatic_Connectivity\Data\Outputs.gdb\Output_Catchments_AlteredWatStreams'
...
arcpy.AddField_management(outWS, 'Dam_Count', "SHORT")
arcpy.AddField_management(outWS, 'Bridge_Count', "SHORT")
arcpy.AddField_management(outWS, 'Culvert_Count', 'SHORT')
arcpy.AddField_management(outWS, 'Stream_Length', 'DOUBLE')
arcpy.AddField_management(outWS, 'Nbrhd_Dam_Count', 'SHORT')
arcpy.AddField_management(outWS, 'Nbrhd_Bridge_Count', 'SHORT')
arcpy.AddField_management(outWS, 'Nbrhd_Culvert_Count', 'SHORT')
arcpy.AddField_management(outWS, 'Nbrhd_Stream_Length', 'DOUBLE')
arcpy.AddField_management(outWS, 'Structure_Density', 'DOUBLE')
arcpy.AddField_management(outWS, 'C_I_AC')
...
Catchments = {}
with arcpy.da.UpdateCursor(outWS, [idField, 'Dam_Count', 'Bridge_Count', 'Culvert_Count', 'Stream_Length', 'Structure_Density', 'UP_COUNT', 'UPADJ_CAT', 'DOWN_CAT']) as cur4:
	for row4 in cur4:
...
with arcpy.da.UpdateCursor(outWS, [idField, 'Nbrhd_Dam_Count', 'Nbrhd_Bridge_Count', 'Nbrhd_Culvert_Count', 'Nbrhd_Stream_Length', 'Structure_Density', 'C_I_AC']) as cur5:
	for row5 in cur5:
		...
		for nbr in nbrList:
			try:
				damList.append(Catchments[nbr][1])
				bridgeList.append(Catchments[nbr][2])
				culvertList.append(Catchments[nbr][3])
				streamList.append(Catchments[nbr][4])
			except:
